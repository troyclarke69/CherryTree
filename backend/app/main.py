        ...
        if res.scalar_one_or_none() is None:
            ...
            session.add(
                models.Content(
                    category="Web",
                    section="Hero",
                    blurb="Clear communication, crisp execution, and thoughtful trade-offs. Would hire again.",
                )
            )
...
async def list_Contents(
    ...
    res = await session.execute(select(models.Content).order_by(models.Content.created_at.desc()))
    rows = res.scalars().all()
    return [
        schemas.ContentOut(
            id=r.id, 
...
@app.post(
    ...
    response_model=schemas.ContentOut,
)
async def admin_create_Content(
    payload: schemas.ContentIn,
    session: AsyncSession = Depends(get_session),
) -> schemas.ContentOut:
    t = models.Content(**payload.model_dump())
    session.add(t)
    await session.commit()
...
@app.delete("/api/admin/Contents/{Content_id}", dependencies=[Depends(require_admin)], status_code=204)
async def admin_delete_Content(
    Content_id: uuid.UUID,
    session: AsyncSession = Depends(get_session),
) -> None:
    await session.execute(delete(models.Content).where(models.Content.id == Content_id))
    await session.commit()
    return None
